...
// Agent: MediaVaultColors adaptive UIColor; View glassPanel glassButtonPrimary glassField glassCard iOS26 glassEffect fallback material; elasticSlide; MediaVaultSpinner; FaviconImageView URLSession no-cache Bearer; Color hex extension.
...
// Agent: ENUM MediaVaultColors static lets; READS UITraitCollection userInterfaceStyle; EXPORTS primary neutral semantic glassStroke divider textOnGradient; CONSUMED across SwiftUI chrome.
enum MediaVaultColors {
...
private struct GlassPanelModifier: ViewModifier {
    ...
    func body(content: Content) -> some View {
        let effectTint = (tint ?? MediaVaultColors.primary500).opacity(tintOpacity)
            ...
            content
                ...
                .overlay(
                    RoundedRectangle(cornerRadius: cornerRadius)
                        .stroke(MediaVaultColors.glassStroke, lineWidth: 1)
                )
        } else {
            content
...
private struct GlassButtonPrimaryModifier: ViewModifier {
    ...
    func body(content: Content) -> some View {
        if #available(iOS 26.0, *) {
            content
                .glassEffect(
                    .regular.tint(MediaVaultColors.primary500.opacity(0.6)),
                    in: RoundedRectangle(cornerRadius: cornerRadius)
                )
                .overlay(
                    RoundedRectangle(cornerRadius: cornerRadius)
            ...
            content
                .background(.thinMaterial, in: RoundedRectangle(cornerRadius: cornerRadius))
                .background(
                    MediaVaultColors.primary600.opacity(colorScheme == .dark ? 0.5 : 0.6),
                    in: RoundedRectangle(cornerRadius: cornerRadius)
                )
                .overlay(
                    RoundedRectangle(cornerRadius: cornerRadius)
    ...
    func body(content: Content) -> some View {
        ...
        if #available(iOS 26.0, *) {
            content
                ...
                .overlay(
                    RoundedRectangle(cornerRadius: cornerRadius)
                        .stroke(MediaVaultColors.primary400.opacity(0.7), lineWidth: 1.5)
                )
        } else {
            content
            ...
            content
                .background {
                    RoundedRectangle(cornerRadius: cornerRadius)
                        .fill(.clear)
                        .glassEffect(
                            .regular.tint(MediaVaultColors.glassFillSubtle),
                            in: RoundedRectangle(cornerRadius: cornerRadius)
                        )
                        .overlay(
                            RoundedRectangle(cornerRadius: cornerRadius)
                                .stroke(MediaVaultColors.glassStrokeSubtle, lineWidth: 1)
                        )
                }
        } else {
            content
...
// Agent: STRUCT MediaVaultSpinner View; STATE isAnimating rotationEffect; ANIMATION linear repeatForever; DEFAULT tint primary400.
struct MediaVaultSpinner: View {
    var tint: Color = MediaVaultColors.primary400
