...
//  so sessions are clearly identified (e.g. "Mobile iOS (Ownly App)").
//
...
enum AppIdentity {
    ...
    /// Format: `Ownly-iOS/{version} ({build}; iOS {osVersion}; {model})`
    /// Example: `Ownly-iOS/1.2 (42; iOS 17.4; iPhone16,1)`
    static let userAgent: String = {
        let appVersion = Bundle.main.infoDictionary?["CFBundleShortVersionString"] as? String ?? "0"
        let buildNumber = Bundle.main.infoDictionary?["CFBundleVersion"] as? String ?? "0"
        let osVersion = ProcessInfo.processInfo.operatingSystemVersionString
        let model = deviceModelIdentifier
        return "Ownly-iOS/\(appVersion) (\(buildNumber); iOS \(osVersion); \(model))"
    }()

    // MARK: - Device metadata (matches web DeviceMetadata shape)

    /// Human-friendly session label shown in "Login sessions".
    /// Example: "Mobile iOS (Ownly App)"
    static let deviceName = "Mobile iOS (Ownly App)"
    ...
    static let deviceBrowser = "Ownly App"

    // MARK: - URLRequest helper

    /// Applies the Ownly iOS User-Agent header to a request.
