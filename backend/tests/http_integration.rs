...
// Human: Admin user creation must reject script clients with no browser origin signals (SEC-012 bootstrap).
// Agent: POST /api/v1/admin/users with admin JWT but no Sec-Fetch-Site or matching Origin; EXPECT 403.
#[tokio::test]
...
// Human: Admin user creation succeeds when Origin matches Host (proxies that strip Sec-Fetch-Site).
// Agent: POST /api/v1/admin/users with matching origin + admin JWT; EXPECT 200.
#[tokio::test]
async fn admin_create_user_allows_matching_origin_without_sec_fetch() {
    let database_url = match std::env::var("DATABASE_URL") {
        Ok(url) if !url.is_empty() => url,
        _ => {
            eprintln!(
                "skipping admin_create_user_allows_matching_origin_without_sec_fetch: DATABASE_URL unset"
            );
            return;
        }
    };
    ...
    let state = match create_test_app_state(&cfg).await {
        ...
        Err(error) => {
            eprintln!(
                "skipping admin_create_user_allows_matching_origin_without_sec_fetch: {error}"
            );
            return;
        }
    };
    ...
    let admin_id = uuid::Uuid::new_v4().to_string();
    let admin_email = format!("admin-origin-guard-{admin_id}@example.com");
    let password_hash =
        ownly_backend::auth::handlers::hash_password("password123").expect("hash password");
    ...
    let created_id = uuid::Uuid::new_v4().to_string();
    let created_email = format!("created-origin-{created_id}@example.com");
    let app = create_router(state.clone());
    let body = json!({
        "email": created_email,
        "password": "password123",
    ...
    let response = app
        .oneshot(
            Request::builder()
                ...
                .header("authorization", format!("Bearer {admin_token}"))
                .header("content-type", "application/json")
                .header("host", "ownly.example.com")
                .header("origin", "https://ownly.example.com")
                .body(Body::from(body.to_string()))
                .unwrap(),
        )
        .await
