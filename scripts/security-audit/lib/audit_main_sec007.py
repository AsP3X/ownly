# Human: Entry orchestration for SEC-007 audit script.
# Agent: READS Sec007Config; CALLS runner_sec007 + report_sec007; RETURNS exit code.
...
from .constants_sec007 import AUDIT_ID
from .report_sec007 import render_report, write_outputs
from .runner_sec007 import run_sec007_audit, validate_target_url
...
def _exit_missing_credentials(cfg, missing: list[str]) -> int:
    ...
    if http.output_format == "json":
        ...
        return 2
    if not http.quiet:
        print("SEC-007: cannot run without owner credentials.", file=sys.stderr)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if err:
        ...
        return 2
    report, _cache = run_sec007_audit(cfg)
