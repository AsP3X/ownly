# Human: Entry orchestration for SEC-009 audit script.
# Agent: READS Sec009Config; CALLS runner_sec009 + report_sec009; RETURNS exit code.
...
from .constants_sec009 import AUDIT_ID
from .report_sec009 import render_report, write_outputs
from .runner_sec009 import run_sec009_audit, validate_target_url
...
def _exit_missing_credentials(cfg, missing: list[str]) -> int:
    ...
    if http.output_format == "json":
        ...
        return 2
    if not http.quiet:
        print("SEC-009: cannot run without owner credentials.", file=sys.stderr)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if err:
        ...
        return 2
    report, _cache = run_sec009_audit(cfg)
