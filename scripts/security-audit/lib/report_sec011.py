# Human: Human-readable and JSON reporting for SEC-011 audits.
...
from .config_sec011 import loaded_env_file_path
from .constants_sec011 import AUDIT_ID, CASE_LABELS, ROUTE_FILES_BULK_DOWNLOAD, ROUTE_FOLDERS
from .models import AuditReport, CaseResult, Config, Sec011Config
from .redact import redact_field_for_display
from .sarif_sec011 import sarif_json


class AuditReporter:
    def __init__(self, cfg: Config, sec011: Sec011Config) -> None:
        self.cfg = cfg
        self.sec011 = sec011
    ...
    def banner(self) -> None:
        title = f"{AUDIT_ID}  Trash zip archive audit"
        inner = self.width - 2
        ...
        print(
            f"  {'Attack':<12} {self.dim(f'owner JWT on {ROUTE_FILES_BULK_DOWNLOAD} and {ROUTE_FOLDERS}/{{id}}/download')}"
        )
    ...
    def summary_block(self, report: AuditReport) -> None:
        ...
        if report.verdict == "vulnerable":
            ...
            print(self.dim("     See security-audit.md → SEC-011"))
...
def render_report(report: AuditReport, sec011: Sec011Config) -> None:
    cfg = sec011.http
    if cfg.quiet:
        return
    ...
    if cfg.output_format == "sarif":
        ...
        return
    AuditReporter(cfg, sec011).print_human(report)
