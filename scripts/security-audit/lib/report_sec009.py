# Human: Human-readable and JSON reporting for SEC-009 audits.
...
from .config_sec009 import loaded_env_file_path
from .constants_sec009 import AUDIT_ID, CASE_LABELS, HEADER_SHARE_PASSWORD, ROUTE_PUBLIC_CONTENTS
from .models import AuditReport, CaseResult, Config, Sec009Config
from .redact import redact_field_for_display
from .sarif_sec009 import sarif_json


class AuditReporter:
    def __init__(self, cfg: Config, sec009: Sec009Config) -> None:
        self.cfg = cfg
        self.sec009 = sec009
    ...
    def banner(self) -> None:
        title = f"{AUDIT_ID}  Share password brute-force audit"
        inner = self.width - 2
        ...
        print(
            f"  {'Attack':<12} {self.dim(f'wrong {HEADER_SHARE_PASSWORD} burst on {ROUTE_PUBLIC_CONTENTS}')}"
        )
        owner = self.sec009.owner_email or self.dim("(SEC009_OWNER_EMAIL)")
        print(f"  {'Owner':<12} {owner}")
        print(f"  {'Attempts':<12} {self.sec009.wrong_attempts} wrong guesses")
    ...
    def summary_block(self, report: AuditReport) -> None:
        ...
        if report.verdict == "vulnerable":
            ...
            print(self.dim("     See security-audit.md → SEC-009"))
...
def render_report(report: AuditReport, sec009: Sec009Config) -> None:
    cfg = sec009.http
    if cfg.quiet:
        return
    ...
    if cfg.output_format == "sarif":
        ...
        return
    AuditReporter(cfg, sec009).print_human(report)
