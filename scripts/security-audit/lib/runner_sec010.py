# Human: SEC-010 — unauthenticated POST /setup/database/test internal Postgres probe.
...
from .evidence_sec010 import build_database_probe_evidence
from .heuristics import json_get
from .heuristics_sec010 import (
...
from .models import AuditReport, CaseResult, HttpResult, Sec010Config


def _http(cfg: Sec010Config):
    return cfg.http
...
def fail_result(
    ...
    *,
    ...
    return CaseResult(
        ...
        remediation=REMEDIATION_SEC010,
    )
...
def _post_database_test(cfg: Sec010Config, database_url: str) -> HttpResult:
    http = _http(cfg)
    return http_post_json(
        http,
        api_url(http, ROUTE_SETUP_DATABASE_TEST),
        {"database_url": database_url},
    )


def test_target_reachable(cfg: Sec010Config, cache: dict[str, Any]) -> CaseResult:
    http = _http(cfg)
...
def test_setup_status_probe(cfg: Sec010Config, cache: dict[str, Any]) -> CaseResult:
...
def test_exploit_primitive(_cfg: Sec010Config, _cache: dict[str, Any]) -> CaseResult:
    return CaseResult(
        name="exploit_primitive_unauthenticated",
        passed=True,
        detail=f"probes POST {ROUTE_SETUP_DATABASE_TEST} without Authorization",
        severity="pass",
    )


def test_pre_setup_required(cfg: Sec010Config, cache: dict[str, Any]) -> CaseResult:
    complete = cache.get("setup_complete")
    if complete is True:
        if cfg.require_pre_setup:
            return CaseResult(
                name="pre_setup_required_for_db_probe",
                passed=False,
                detail="setup_complete=true — use uninitialized instance or fresh DB to test DB probe",
                severity="error",
            )
        return CaseResult(
            name="pre_setup_required_for_db_probe",
            passed=True,
            detail="setup_complete=true — DB probes skipped; post-setup gating checked",
            severity="pass",
        )
    if complete is False:
        return CaseResult(
            name="pre_setup_required_for_db_probe",
            passed=True,
            detail="setup_complete=false (pre-setup DB probe applicable)",
            severity="pass",
        )
    return CaseResult(
        name="pre_setup_required_for_db_probe",
...
def test_post_setup_gated(cfg: Sec010Config, cache: dict[str, Any]) -> CaseResult:
    if cache.get("setup_complete") is not True:
        return CaseResult(
            name="post_setup_database_test_gated",
            passed=True,
            detail="skipped (pre-setup)",
            severity="pass",
        )
    label, url = cfg.probe_targets[0]
    res = _post_database_test(cfg, url)
    cache["post_setup_probe"] = res
    _record_timing(cache, "post_setup_probe", res)
    if database_test_setup_complete_block(res):
        return CaseResult(
            name="post_setup_database_test_gated",
            passed=True,
            detail=f"database test blocked after setup (HTTP {res.status}, {label})",
            severity="pass",
        )
    if database_test_auth_required(res):
        return CaseResult(
            name="post_setup_database_test_gated",
            passed=True,
            detail=f"database test requires auth after setup (HTTP {res.status})",
            severity="pass",
        )
    return CaseResult(
        name="post_setup_database_test_gated",
...
def test_database_test_public_pre_setup(cfg: Sec010Config, cache: dict[str, Any]) -> CaseResult:
    if cache.get("setup_complete") is not False:
        return CaseResult(
            name="database_test_public_pre_setup",
            passed=True,
    ...
    if database_test_auth_required(res):
        return CaseResult(
            name="database_test_public_pre_setup",
            passed=True,
            detail="database test requires bootstrap auth",
            severity="pass",
        )
    if unauthenticated_database_probe_accepted(res):
        return CaseResult(
            name="database_test_public_pre_setup",
            passed=True,
            detail=f"unauthenticated database test accepted (HTTP {res.status}, {label})",
            severity="pass",
        )
    return CaseResult(
        name="database_test_public_pre_setup",
...
def test_internal_targets_not_blocked(cfg: Sec010Config, cache: dict[str, Any]) -> CaseResult:
    if cache.get("setup_complete") is not False:
        return CaseResult(
    ...
    for label, url in cfg.probe_targets:
        ...
        if db_probe_vulnerable(res):
            vulnerable.append(label)
    cache["probe_results"] = probe_results
    if vulnerable:
        first = vulnerable[0]
        cache["evidence_database_probe"] = build_database_probe_evidence(
            probe_results[first],
            route=ROUTE_SETUP_DATABASE_TEST,
            title="Internal database URL accepted",
            target_label=first,
        )
        return fail_result(
            "internal_targets_not_blocked",
            f"internal/private database URLs not rejected: {', '.join(vulnerable)}",
            evidence_key="database_probe",
        )
    return CaseResult(
        name="internal_targets_not_blocked",
        passed=True,
...
def run_sec010_audit(cfg: Sec010Config) -> tuple[AuditReport, dict[str, Any]]:
    cache: dict[str, Any] = {}
    ...
    steps: list[tuple[str, Callable[[], CaseResult]]] = [
        ...
        ("public", lambda: test_database_test_public_pre_setup(cfg, cache)),
    ...
    evidence = {}
    if "evidence_database_probe" in cache:
        evidence["database_probe"] = cache["evidence_database_probe"]

    hints = []
    if fails:
        hints.append(REMEDIATION_SEC010)
...
def validate_target_url(cfg: Sec010Config) -> str | None:
