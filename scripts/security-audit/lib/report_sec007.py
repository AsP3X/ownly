# Human: Human-readable and JSON reporting for SEC-007 audits.
...
from .config_sec007 import loaded_env_file_path
from .constants_sec007 import AUDIT_ID, CASE_LABELS, ROUTE_PUBLIC_OVERVIEW
from .models import AuditReport, CaseResult, Config, Sec007Config
from .redact import redact_field_for_display
from .sarif_sec007 import sarif_json


class AuditReporter:
    def __init__(self, cfg: Config, sec007: Sec007Config) -> None:
        self.cfg = cfg
        self.sec007 = sec007
    ...
    def banner(self) -> None:
        title = f"{AUDIT_ID}  Share overview password bypass audit"
        inner = self.width - 2
        ...
        print(
            f"  {'Attack':<12} {self.dim(f'GET {ROUTE_PUBLIC_OVERVIEW} without x-share-password')}"
        )
        owner = self.sec007.owner_email or self.dim("(SEC007_OWNER_EMAIL)")
    ...
    def summary_block(self, report: AuditReport) -> None:
        ...
        if report.verdict == "vulnerable":
            ...
            print(self.dim("     See security-audit.md → SEC-007"))
...
def render_report(report: AuditReport, sec007: Sec007Config) -> None:
    cfg = sec007.http
    if cfg.quiet:
        return
    ...
    if cfg.output_format == "sarif":
        ...
        return
    AuditReporter(cfg, sec007).print_human(report)
