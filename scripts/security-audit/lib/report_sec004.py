# Human: Human-readable and JSON reporting for SEC-004 audits.
...
from .config_sec004 import loaded_env_file_path
from .constants_sec004 import AUDIT_ID, CASE_LABELS, ROUTE_SETUP_STATUS
from .models import AuditReport, CaseResult, Config, LeakEvidence, Sec004Config
from .redact import redact_field_for_display
from .sarif_sec004 import sarif_json


class AuditReporter:
    def __init__(self, cfg: Config, sec004: Sec004Config) -> None:
        self.cfg = cfg
        self.sec004 = sec004
    ...
    def banner(self) -> None:
        title = f"{AUDIT_ID}  Authenticated trash download audit"
        inner = self.width - 2
        ...
        print(f"  {'Attack':<12} {self.dim('owner JWT → trash file → download / download-url / preview-url')}")
        owner = self.sec004.owner_email or self.dim("(SEC004_OWNER_EMAIL)")
    ...
    def summary_block(self, report: AuditReport) -> None:
        ...
        if report.verdict == "vulnerable":
            ...
            print(self.dim("     See security-audit.md → SEC-004"))
...
def render_report(report: AuditReport, sec004: Sec004Config) -> None:
    cfg = sec004.http
    if cfg.quiet:
        return
    ...
    if cfg.output_format == "sarif":
        ...
        return
    AuditReporter(cfg, sec004).print_human(report)
