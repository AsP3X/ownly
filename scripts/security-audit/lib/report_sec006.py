# Human: Human-readable and JSON reporting for SEC-006 audits.
...
from .config_sec006 import loaded_env_file_path
from .constants_sec006 import AUDIT_ID, CASE_LABELS, ROUTE_AUTH_LOGIN
from .models import AuditReport, CaseResult, Config, Sec006Config
from .redact import redact_field_for_display
from .sarif_sec006 import sarif_json


class AuditReporter:
    def __init__(self, cfg: Config, sec006: Sec006Config) -> None:
        self.cfg = cfg
        self.sec006 = sec006
    ...
    def banner(self) -> None:
        title = f"{AUDIT_ID}  Rate-limit forwarded-header audit"
        inner = self.width - 2
        ...
        print(
            f"  {'Attack':<12} {self.dim(f'rotate X-Forwarded-For on {ROUTE_AUTH_LOGIN} bursts')}"
        )
        print(
            f"  {'Limits':<12} {self.dim(f'login {self.sec006.login_rpm}/min, register {self.sec006.register_rpm}/min')}"
        )
    ...
    def summary_block(self, report: AuditReport) -> None:
        ...
        if report.verdict == "vulnerable":
            ...
            print(self.dim("     See security-audit.md → SEC-006"))
...
def render_report(report: AuditReport, sec006: Sec006Config) -> None:
    cfg = sec006.http
    if cfg.quiet:
        return
    ...
    if cfg.output_format == "sarif":
        ...
        return
    AuditReporter(cfg, sec006).print_human(report)
