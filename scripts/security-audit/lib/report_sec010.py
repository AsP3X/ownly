# Human: Human-readable and JSON reporting for SEC-010 audits.
...
from .config_sec010 import loaded_env_file_path
from .constants_sec010 import AUDIT_ID, CASE_LABELS, ROUTE_SETUP_DATABASE_TEST
from .models import AuditReport, CaseResult, Config, Sec010Config
from .redact import redact_field_for_display
from .sarif_sec010 import sarif_json


class AuditReporter:
    def __init__(self, cfg: Config, sec010: Sec010Config) -> None:
        self.cfg = cfg
        self.sec010 = sec010
    ...
    def banner(self) -> None:
        title = f"{AUDIT_ID}  Setup database probe audit"
        inner = self.width - 2
        ...
        print(
            f"  {'Attack':<12} {self.dim(f'unauthenticated POST {ROUTE_SETUP_DATABASE_TEST}')}"
        )
    ...
    def summary_block(self, report: AuditReport) -> None:
        ...
        if report.verdict == "vulnerable":
            ...
            print(self.dim("     See security-audit.md → SEC-010"))
        elif report.verdict == "inconclusive":
            print(f"  {self.yellow('?  INCONCLUSIVE')}")
        else:
            print(f"  {self.green('✓  OK')}")
        if report.setup_complete is True:
            print()
            print(
                self.dim(
                    "  Note: DB probes need setup_complete=false; post-setup gating was checked separately."
                )
            )
...
def render_report(report: AuditReport, sec010: Sec010Config) -> None:
    cfg = sec010.http
    if cfg.quiet:
        return
    ...
    if cfg.output_format == "sarif":
        ...
        return
    AuditReporter(cfg, sec010).print_human(report)
