# Human: Human-readable and JSON reporting for SEC-008 audits.
...
from .config_sec008 import loaded_env_file_path
from .constants_sec008 import AUDIT_ID, CASE_LABELS, ROUTE_SETUP_STORAGE_TEST
from .models import AuditReport, CaseResult, Config, Sec008Config
from .redact import redact_field_for_display
from .sarif_sec008 import sarif_json


class AuditReporter:
    def __init__(self, cfg: Config, sec008: Sec008Config) -> None:
        self.cfg = cfg
        self.sec008 = sec008
    ...
    def banner(self) -> None:
        title = f"{AUDIT_ID}  Setup storage SSRF audit"
        inner = self.width - 2
        ...
        print(
            f"  {'Attack':<12} {self.dim(f'unauthenticated POST {ROUTE_SETUP_STORAGE_TEST}')}"
        )
    ...
    def summary_block(self, report: AuditReport) -> None:
        ...
        if report.verdict == "vulnerable":
            ...
            print(self.dim("     See security-audit.md → SEC-008"))
        ...
        if report.setup_complete is True:
            ...
            print(
                self.dim(
                    "  Note: SSRF probes need setup_complete=false; post-setup gating was checked separately."
                )
            )
...
def render_report(report: AuditReport, sec008: Sec008Config) -> None:
    cfg = sec008.http
    if cfg.quiet:
        return
    ...
    if cfg.output_format == "sarif":
        ...
        return
    AuditReporter(cfg, sec008).print_human(report)
