    ...
    >
      ...
      {!minimized ? (
        <div className="flex flex-col gap-3 px-4 py-3">
          ...
          {job.currentNodeId ? (
            <p className="text-xs text-ink-muted">Node: {job.currentNodeId}</p>
          ) : job.nodeId ? (
            <p className="text-xs text-ink-muted">Node: {job.nodeId}</p>
          ) : null}
          {job.prefix ? (
            <p className="text-xs text-ink-muted">Prefix: {job.prefix}</p>
          ) : null}

          {job.lastNodeSummaries.length > 0 ? (
            <ul className="max-h-24 list-none space-y-1 overflow-y-auto text-xs text-ink-muted">
