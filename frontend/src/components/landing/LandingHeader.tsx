...
export function LandingHeader() {
  ...
  return (
    <header className="flex h-16 w-full items-center justify-between">
      <Link to="/" className="flex shrink-0 items-center touch:min-h-11" aria-label="Ownly home">
        <LandingBrandLogo />
      </Link>
      ...
      <div className="flex items-center gap-5">
        <Link
          to="/login"
          className="text-sm font-semibold text-[#1A1A1A] transition-colors hover:text-[#2563EB] touch:inline-flex touch:min-h-11 touch:items-center touch:px-1"
        >
          Sign In
        </Link>
        <Link
          to="/register"
          className="rounded-lg bg-[#2563EB] px-[18px] py-2.5 text-sm font-bold text-white transition-colors hover:bg-[#1d4ed8] touch:inline-flex touch:min-h-11 touch:items-center"
        >
          Get Started
        </Link>
      </div>
